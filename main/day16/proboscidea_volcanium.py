...
def traverse(current, num_turns, current_turn, open_valves, flow_rate):
    ...
    if current.id in positive_flow_rates and current.id not in open_valves:
        # Stay where you are and open the valve, costing one move.
        # Don't count the new valve yet
        max_flow_rate = max(
            traverse(current, num_turns, current_turn + 1,
                     open_valves.union((current.id,)),
                     calc_flow_rate(open_valves, flow_rate)), max_flow_rate)
    for neighbour in current.children:
        max_flow_rate = max(
            traverse(valves[neighbour], num_turns, current_turn + 1,
                     open_valves,
                     calc_flow_rate(open_valves, flow_rate)), max_flow_rate)
